...
AccelStepper myStepper(MotorInterfaceType, 33, 25, 26, 27);
/*********/
...
void setup() {
  ...
  myStepper.setMaxSpeed(800);    // max speed in steps per second
  myStepper.setAcceleration(600); // acceleration in steps per second^2
...
void loop() {
  ...
  Serial.println("Driving Stepper motor UP");
  while(digitalRead(HALL_IN_PIN) == HIGH){
    myStepper.setSpeed(800); 
    myStepper.runSpeed();
  }
  delay(1000);

  Serial.println("Driving Stepper motor DOWN");
  while(digitalRead(HALL_OUT_PIN) == HIGH){
    myStepper.setSpeed(-800);
    myStepper.runSpeed();
  }
  delay(1000);

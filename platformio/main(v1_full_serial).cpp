...
int32_t sensor_data[4]; // [mprls1, mprls2, mprls3, tof]
...
void sensor_timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
  ...
  sensor_data[0] = (int32_t)(read_mprls_sensor(0));
  sensor_data[1] = (int32_t)(read_mprls_sensor(1));
  sensor_data[2] = (int32_t)(read_mprls_sensor(2));
  sensor_data[3] = (int32_t)(read_tof_sensor(3));
...
void setup() {
  ...
  sensor_msg.data.data = (int32_t*) malloc(4 * sizeof(int32_t));
